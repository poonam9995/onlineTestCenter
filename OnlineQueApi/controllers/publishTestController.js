            ...
            for (let i = 0; i < response.candidateList.length; i++) {
                ...
                const mailOptions = {
                    ...
                    text: `  User Id : ${response.candidateList[i].Email}
                         ...
                         http://localhost:4200/#/user/login click here to get the URL`// http://localhost:4200/#/user/login/${response._id} click here to get the URL plain text body
                };
...
exports.updateResult = (req, res) => {
    ...
    for (let i = 0; i < testResult.length; i++) {
        candidateAnswer.push({
            id: testResult[i].id,
            status: testResult[i].status,
            candidateAns: testResult[i].CandidateAnswer
        });
    }
  //      publishTest.findOneAndUpdate
    console.log(candidateAnswer,testResult[testResult.length - 1].correctMarks,req.body.status)
    publishTest.updateOne(
        { _id: req.body.id, "candidateList.Email": req.userData.email, "candidateList.Password": req.userData.authId },
        { $set:
            {
            "candidateList.$[element]":
            {              
                    "result": {
                        "marks": testResult[testResult.length - 1].correctMarks,
                ...
                {
            multi: true,
            arrayFilters: [{ "element": { "candidateList.Email": req.userData.email}}]
        }                  
